...
static ESP_PanelLcd   *lcd_panel   = NULL;
static ESP_PanelTouch *touch_panel = NULL;

// Same init sequence as the manufacturer demo (ST77916)
static const esp_lcd_panel_vendor_init_cmd_t lcd_init_cmds[] = {
...
static IRAM_ATTR bool flush_ready_cb(void *user_data)
{
...
{
    ESP_PanelTouch *tp = (ESP_PanelTouch *)drv->user_data;
    ESP_PanelTouchPoint point;
    int count = tp->readPoints(&point, 1);
    if (count > 0) {
...
extern "C" void lcd_lvgl_Init(void)
{
    ...
    panel_bus->configQspiFreqHz(50 * 1000 * 1000);
    panel_bus->begin();

    lcd_panel = new ESP_PanelLcd_ST77916(panel_bus, LCD_BIT_PER_PIXEL, EXAMPLE_PIN_NUM_LCD_RST);
    lcd_panel->configVendorCommands(lcd_init_cmds, sizeof(lcd_init_cmds) / sizeof(lcd_init_cmds[0]));
    lcd_panel->init();
    lcd_panel->reset();
    lcd_panel->begin();
    lcd_panel->invertColor(true);
    lcd_panel->displayOn();

    // --- Touch (CST816S via I2C) ---
    ESP_PanelBusI2C *touch_bus = new ESP_PanelBusI2C(
    ...
    touch_panel = new ESP_PanelTouch_CST816S(
        touch_bus,
        EXAMPLE_LCD_H_RES,
        EXAMPLE_LCD_V_RES,
    ...
    // Notify LVGL when DMA transfer finishes
    lcd_panel->attachDrawBitmapFinishCallback(flush_ready_cb, (void *)disp->driver);
    ...
    indev_drv.user_data = touch_panel;
